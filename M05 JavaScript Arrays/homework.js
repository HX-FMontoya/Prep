...
function agregarNumeros(arrayOfNums) {
  ...
  // Tu código:
  return arrayOfNums.reduce((prev, next) => prev + next);
}
...
function promedioResultadosTest(resultadosTest) {
  ...
  // Tu código:
  return (
    resultadosTest.reduce((prev, next) => prev + next) / resultadosTest.length
  );
}
...
function numeroMasGrande(arrayOfNums) {
  ...
  for (let i = 0; i < arrayOfNums.length; i++) {
    if (arrayOfNums[i] > major) major = arrayOfNums[i];
  }
  return major;
}
